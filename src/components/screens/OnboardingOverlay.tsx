...
export function OnboardingOverlay({ isOpen, onClose, onFinish }: OnboardingOverlayProps) {
  ...
  const slides = [
    { icon: '🎮', title: t.onboardingSlides.move.title, desc: t.onboardingSlides.move.desc },
    { icon: '🍎', title: t.onboardingSlides.food.title, desc: t.onboardingSlides.food.desc },
    { icon: '⚡', title: t.onboardingSlides.powerup.title, desc: t.onboardingSlides.powerup.desc },
  ]
    ...
    >
      ...
      >
        ...
        <div className="mb-6 text-center">
          ...
          <p className="mt-2 text-slate-300 text-sm" dangerouslySetInnerHTML={{ __html: slide.desc }} />
        </div>
        ...
        <div className="flex justify-center gap-2 mb-4">
          {slides.map((_, i) => (
            <button
              ...
              aria-label={`${t.onboardingSlides.move.title} ${i + 1}`}
            />
          ))}
        </div>
